...
<html>
      <head>
            <title>Edit Details Form page</title>
            <meta charset="UTF-8">
      ...
      <body>
			 ...
			 <div class="row">
			 <div class="col-md-6">
             <h2 style="margin-top:65px; font-size:42px; font-family:Times New Roman">Edit Details</h2><br>
             <form method = "post" action = "editform.php">
			     <label for="pid" style="font-size:18px">Enter id</label>
				 <input type="text" id="pid" name="pid" style="width:200px; height:30px; font-size:16px; margin-left:153px" required> <br><br>
...
  				 <label for="NA" style="font-weight:normal; font-size:16px">NA</label> <br>
				 ...
				 <input type="submit" value="Edit" class="btn btn-primary" style="margin:20px 0 0 0; font-size:18px; width:70px"> <br><br><br>
				 </div>
			 </form>
			 </div>
			 </div>
			...
			<?php
				...
				if(isset($_POST['pid']) && isset($_POST['fac']) && isset($_POST['type']) && isset($_POST['name']) && isset($_POST['address']) && isset($_POST['city']) && 
				...
				isset($_POST['link']) && isset($_POST['otherinfo']))
				{
					if(strcmp($_POST['oo'],"online")==0)
					{
						...
						{
							$faccoll->updateOne(['id' => $pid], ['$set' => ["facility" => $fac, "type" => $type, "name" => $name, "address" => $add, "city" => $city, 
							"pincode" => $pincode, "state" => $state, "phone number" => $phone, "availability" => $av, "monetary details" => $monetary, "booking" => $oo, 
							"booking link" => $link, "other info" => $otherinfo, "owneremail" => $mail]]);
						}
						else
						{
							$faccoll->updateOne(['id' => $pid], ['$set' => ["facility" => $fac, "type" => $type, "name" => $name, "address" => $add, "city" => $city, 
							"pincode" => $pincode, "state" => $state, "phone number" => $phone, "monetary details" => $monetary, "booking" => $oo, 
							"booking link" => $link, "other info" => $otherinfo, "owneremail" => $mail]]);
						}
					}
					else
					{
						if(strcmp($_POST['fac'],"Accommodation")==0)
						{
							$faccoll->updateOne(['id' => $pid], ['$set' => ["facility" => $fac, "type" => $type, "name" => $name, "address" => $add, "city" => $city, 
							"pincode" => $pincode, "state" => $state, "phone number" => $phone, "availability" => $av, "monetary details" => $monetary, "booking" => $oo, 
							"booking link" => "NA", "other info" => $otherinfo, "owneremail" => $mail]]);
						}
						else
						{
							$faccoll->updateOne(['id' => $pid], ['$set' => ["facility" => $fac, "type" => $type, "name" => $name, "address" => $add, "city" => $city, 
